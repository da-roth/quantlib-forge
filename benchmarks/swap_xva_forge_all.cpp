...
// =============================================================================
// XVA PERFORMANCE TEST - COMPREHENSIVE FORGE BENCHMARK (ALL APPROACHES)
// =============================================================================
// This test compares ALL Forge approaches in one comprehensive benchmark:
//   1. Bump-Reval (baseline)                    - Direct QuantLib evaluation with finite differences
//   2. Forge Kernel Bump-Reval                  - JIT-compiled kernel, still using bump-reval (markForgeInputAndDiff)
//   3. Forge Forward SSE2 Stability             - JIT-compiled kernel, forward-only, stability-only (markForgeInput)
//   4. Forge Forward SSE2 AllOpt                - JIT-compiled kernel, forward-only, all optimizations (markForgeInput)
//   5. Forge Forward AVX2 Stability             - JIT-compiled kernel, forward-only, AVX2, stability-only (markForgeInput)
//   6. Forge Forward AVX2 AllOpt                - JIT-compiled kernel, forward-only, AVX2, all optimizations (markForgeInput)
//   7. Forge AAD SSE2 Stability                 - JIT-compiled kernel with AAD, stability-only optimizations
//   8. Forge AAD SSE2 AllOpt                    - JIT-compiled kernel with AAD, all optimizations
//   9. Forge AAD SSE2 NoOpt                     - JIT-compiled kernel with AAD, no optimizations
//  10. Forge AAD AVX2 Stability                 - JIT-compiled kernel with AAD, AVX2, stability-only (if supported)
//  11. Forge AAD AVX2 AllOpt                    - JIT-compiled kernel with AAD, AVX2, all optimizations (if supported)
//  12. Forge AAD AVX2 NoOpt                     - JIT-compiled kernel with AAD, AVX2, no optimizations (if supported)
//
// This provides a comprehensive comparison of all optimization levels and instruction sets.
// =============================================================================
...
namespace {
    ...
    //    Uses markForgeInputAndDiff() but still computes sensitivities via bump-reval
    //=========================================================================
    ...
    //=========================================================================
    // 3. FORGE FORWARD BUMP-REVAL (JIT, no AAD - uses markForgeInput)
    //=========================================================================
    ...
    //=========================================================================
    // 4. FORGE AAD COMPUTATION - SSE2 (Scalar, 1 scenario per execution)
    //=========================================================================
    ...
    //=========================================================================
    // Forward-only wrapper functions (markForgeInput, no AAD buffers)
    //=========================================================================
    ...
    // AAD SSE2 / AVX2 variants
    //=========================================================================
    // AAD-SSE2 (stability-only)
    ...
    // AAD-SSE2-Optimized (all optimizations)
    ...
    // AAD-AVX2-Optimized (all optimizations)
    ...
    //=========================================================================
    // Print results table (Bump + Fwd-only + AAD, SSE2 vs AVX2)
    //=========================================================================
    void printResultsTable(
        const std::string& testName,
        const TimingResults& bumpTiming,
        const XvaResults& bumpResults,
        // Forward SSE2 variants
        const TimingResults& forwardSSE2StabTiming,
        const XvaResults& forwardSSE2StabResults,
        const TimingResults& forwardSSE2AllOptTiming,
        const XvaResults& forwardSSE2AllOptResults,
        // Forward AVX2 variants (nullptr if not available)
        const TimingResults* forwardAVX2StabTiming,
        const XvaResults* forwardAVX2StabResults,
        const TimingResults* forwardAVX2AllOptTiming,
        const XvaResults* forwardAVX2AllOptResults,
        // AAD SSE2 variants
        const TimingResults& aadSSE2StabTiming,
        const XvaResults& aadSSE2StabResults,
        const TimingResults& aadSSE2AllOptTiming,
        const XvaResults& aadSSE2AllOptResults,
        // AAD AVX2 variants (nullptr if not available)
        const TimingResults* aadAVX2StabTiming,
        const XvaResults* aadAVX2StabResults,
        const TimingResults* aadAVX2AllOptTiming,
        const XvaResults* aadAVX2AllOptResults,
        const std::vector<IRPillar>& eurPillars,
        const XvaConfig& config) {

        const int col0 = 22;  // Label column
        const int colW = 12;  // Data columns

        bool hasAvx2 = (aadAVX2StabTiming != nullptr);

        auto line = [&]() {
            std::cout << "+" << std::string(col0, '-');
            for (int i = 0; i < (hasAvx2 ? 12 : 9); ++i)
                std::cout << "+" << std::string(colW, '-');
            std::cout << "+\n";
        };

        int numCols = hasAvx2 ? 9 : 5;
        ...
        std::cout << "|" << std::setw(col0) << std::left << " Method"
                  ...
                  << "|" << std::setw(colW) << std::right << "F-Fwd"
                  << "|" << std::setw(colW) << std::right << "F-Fwd-Opt";
        if (hasAvx2) {
            std::cout << "|" << std::setw(colW) << std::right << "F-Fwd-AVX"
                      << "|" << std::setw(colW) << std::right << "F-Fwd-AVX-Opt";
        }
        std::cout << "|" << std::setw(colW) << std::right << "AAD-S2"
                  << "|" << std::setw(colW) << std::right << "AAD-S2-Opt";
        if (hasAvx2) {
            std::cout << "|" << std::setw(colW) << std::right << "AAD-A2"
                      << "|" << std::setw(colW) << std::right << "AAD-A2-Opt";
        }
        std::cout << "|\n";
        line();
        ...
        auto printRow = [&](const std::string& label,
                           double bump, double fwd, double fwdOpt,
                           double fwdAvx, double fwdAvxOpt,
                           double aadS2, double aadS2Opt,
                           double aadA2, double aadA2Opt,
                           bool bumpDash = false) {
            ...
            std::cout << "|" << std::setw(colW) << std::right << fwd
                      << "|" << std::setw(colW) << std::right << fwdOpt;
            if (hasAvx2) {
                std::cout << "|" << std::setw(colW) << std::right << fwdAvx
                          << "|" << std::setw(colW) << std::right << fwdAvxOpt;
            }
            std::cout << "|" << std::setw(colW) << std::right << aadS2
                      << "|" << std::setw(colW) << std::right << aadS2Opt;
            if (hasAvx2) {
                std::cout << "|" << std::setw(colW) << std::right << aadA2
                          << "|" << std::setw(colW) << std::right << aadA2Opt;
            }
            std::cout << "|\n";
        };

        // Show average runtime per timed iteration (total wall-clock / timedRuns)
        printRow(" Avg Time/Iter (ms)", bumpTiming.avgTimePerIterationMs,
                 forwardSSE2StabTiming.avgTimePerIterationMs, forwardSSE2AllOptTiming.avgTimePerIterationMs,
                 hasAvx2 ? forwardAVX2StabTiming->avgTimePerIterationMs : 0,
                 hasAvx2 ? forwardAVX2AllOptTiming->avgTimePerIterationMs : 0,
                 aadSSE2StabTiming.avgTimePerIterationMs, aadSSE2AllOptTiming.avgTimePerIterationMs,
                 hasAvx2 ? aadAVX2StabTiming->avgTimePerIterationMs : 0,
                 hasAvx2 ? aadAVX2AllOptTiming->avgTimePerIterationMs : 0);

        printRow(" Pure Eval (ms)", bumpTiming.evaluationTimeMs,
                 forwardSSE2StabTiming.evaluationTimeMs, forwardSSE2AllOptTiming.evaluationTimeMs,
                 hasAvx2 ? forwardAVX2StabTiming->evaluationTimeMs : 0,
                 hasAvx2 ? forwardAVX2AllOptTiming->evaluationTimeMs : 0,
                 aadSSE2StabTiming.evaluationTimeMs, aadSSE2AllOptTiming.evaluationTimeMs,
                 hasAvx2 ? aadAVX2StabTiming->evaluationTimeMs : 0,
                 hasAvx2 ? aadAVX2AllOptTiming->evaluationTimeMs : 0);

        printRow(" Kernel Create (ms)", 0.0,
                 forwardSSE2StabTiming.kernelCreationTimeMs, forwardSSE2AllOptTiming.kernelCreationTimeMs,
                 hasAvx2 ? forwardAVX2StabTiming->kernelCreationTimeMs : 0.0,
                 hasAvx2 ? forwardAVX2AllOptTiming->kernelCreationTimeMs : 0.0,
                 aadSSE2StabTiming.kernelCreationTimeMs, aadSSE2AllOptTiming.kernelCreationTimeMs,
                 hasAvx2 ? aadAVX2StabTiming->kernelCreationTimeMs : 0.0,
                 hasAvx2 ? aadAVX2AllOptTiming->kernelCreationTimeMs : 0.0,
                 true);
        ...
        auto safeSpeedup = [&](double base, double other) {
            return (other > 0.0) ? base / other : 0.0;
        };

        double spFwd      = safeSpeedup(bumpTiming.avgTimePerIterationMs, forwardSSE2StabTiming.avgTimePerIterationMs);
        double spFwdOpt   = safeSpeedup(bumpTiming.avgTimePerIterationMs, forwardSSE2AllOptTiming.avgTimePerIterationMs);
        double spFwdAvx   = hasAvx2 ? safeSpeedup(bumpTiming.avgTimePerIterationMs, forwardAVX2StabTiming->avgTimePerIterationMs) : 0.0;
        double spFwdAvxOp = hasAvx2 ? safeSpeedup(bumpTiming.avgTimePerIterationMs, forwardAVX2AllOptTiming->avgTimePerIterationMs) : 0.0;
        double spAadS2    = safeSpeedup(bumpTiming.avgTimePerIterationMs, aadSSE2StabTiming.avgTimePerIterationMs);
        double spAadS2Op  = safeSpeedup(bumpTiming.avgTimePerIterationMs, aadSSE2AllOptTiming.avgTimePerIterationMs);
        double spAadA2    = hasAvx2 ? safeSpeedup(bumpTiming.avgTimePerIterationMs, aadAVX2StabTiming->avgTimePerIterationMs) : 0.0;
        double spAadA2Op  = hasAvx2 ? safeSpeedup(bumpTiming.avgTimePerIterationMs, aadAVX2AllOptTiming->avgTimePerIterationMs) : 0.0;

        std::cout << "|" << std::setw(colW-1) << std::right << spFwd << "x"
                  << "|" << std::setw(colW-1) << std::right << spFwdOpt << "x";
        if (hasAvx2) {
            std::cout << "|" << std::setw(colW-1) << std::right << spFwdAvx << "x"
                      << "|" << std::setw(colW-1) << std::right << spFwdAvxOp << "x";
        }
        std::cout << "|" << std::setw(colW-1) << std::right << spAadS2 << "x"
                  << "|" << std::setw(colW-1) << std::right << spAadS2Op << "x";
        if (hasAvx2) {
            std::cout << "|" << std::setw(colW-1) << std::right << spAadA2 << "x"
                      << "|" << std::setw(colW-1) << std::right << spAadA2Op << "x";
        }
        std::cout << "|\n";
        line();
        ...
        std::cout << "|" << std::setw(col0) << std::left << " Expected Exposure"
                  ...
                  << "|" << std::setw(colW) << std::right << forwardSSE2StabResults.expectedExposure
                  << "|" << std::setw(colW) << std::right << forwardSSE2AllOptResults.expectedExposure;
        if (hasAvx2) {
            std::cout << "|" << std::setw(colW) << std::right << forwardAVX2StabResults->expectedExposure
                      << "|" << std::setw(colW) << std::right << forwardAVX2AllOptResults->expectedExposure;
        }
        std::cout << "|" << std::setw(colW) << std::right << aadSSE2StabResults.expectedExposure
                  << "|" << std::setw(colW) << std::right << aadSSE2AllOptResults.expectedExposure;
        if (hasAvx2) {
            std::cout << "|" << std::setw(colW) << std::right << aadAVX2StabResults->expectedExposure
                      << "|" << std::setw(colW) << std::right << aadAVX2AllOptResults->expectedExposure;
        }
        std::cout << "|\n";

        std::cout << "|" << std::setw(col0) << std::left << " CVA"
                  << "|" << std::setw(colW) << std::right << bumpResults.cva
                  << "|" << std::setw(colW) << std::right << aadSSE2StabResults.cva  // reuse columns for exposures
                  << "|" << std::setw(colW) << std::right << aadSSE2AllOptResults.cva;
        if (hasAvx2) {
            std::cout << "|" << std::setw(colW) << std::right << aadAVX2StabResults->cva
                      << "|" << std::setw(colW) << std::right << aadAVX2AllOptResults->cva;
        }
        std::cout << "|" << std::setw(colW) << std::right << aadSSE2StabResults.cva
                  << "|" << std::setw(colW) << std::right << aadSSE2AllOptResults.cva;
        if (hasAvx2) {
            std::cout << "|" << std::setw(colW) << std::right << aadAVX2StabResults->cva
                      << "|" << std::setw(colW) << std::right << aadAVX2AllOptResults->cva;
...
int main(int argc, char* argv[]) {
    std::cout << "=============================================================\n";
    std::cout << "  XVA Benchmark - QuantLib with Forge (COMPREHENSIVE)\n";
    std::cout << "  All approaches: Bump-Reval, Forward-Only, AAD\n";
    std::cout << "  SSE2 vs AVX2, Stability vs AllOpt vs NoOpt\n";
    std::cout << "=============================================================\n";
    ...
    try {
        ...
        for (const auto& config : testCases) {
            ...
            // 2. Forward SSE2 variants
            std::cout << "  Forward-SSE2-Stab (" << config.numRiskFactors << " RF)...\n";
            XvaResults forwardSSE2StabResults;
            auto forwardSSE2StabTiming = runWithTiming(computeForgeForwardSSE2Stability, config, swaps, scenarios, eurPillars, today, calendar, dayCounter, forwardSSE2StabResults);

            std::cout << "  Forward-SSE2-AllOpt (" << config.numRiskFactors << " RF)...\n";
            XvaResults forwardSSE2AllOptResults;
            auto forwardSSE2AllOptTiming = runWithTiming(computeForgeForwardSSE2AllOpt, config, swaps, scenarios, eurPillars, today, calendar, dayCounter, forwardSSE2AllOptResults);

            // 3. Forward AVX2 variants (if supported)
            XvaResults forwardAVX2StabResults, forwardAVX2AllOptResults;
            TimingResults forwardAVX2StabTiming, forwardAVX2AllOptTiming;
            if (avx2Supported) {
                std::cout << "  Forward-AVX2-Stab (" << config.numRiskFactors << " RF)...\n";
                forwardAVX2StabTiming = runWithTiming(computeForgeForwardAVX2Stability, config, swaps, scenarios, eurPillars, today, calendar, dayCounter, forwardAVX2StabResults);

                std::cout << "  Forward-AVX2-AllOpt (" << config.numRiskFactors << " RF)...\n";
                forwardAVX2AllOptTiming = runWithTiming(computeForgeForwardAVX2AllOpt, config, swaps, scenarios, eurPillars, today, calendar, dayCounter, forwardAVX2AllOptResults);
            }

            // 4. AAD SSE2 variants
            std::cout << "  AAD-SSE2-Stab (" << config.numRiskFactors << " RF)...\n";
            XvaResults aadSSE2StabResults;
            auto aadSSE2StabTiming = runWithTiming(computeForgeAadSSE2Stability, config, swaps, scenarios, eurPillars, today, calendar, dayCounter, aadSSE2StabResults);

            std::cout << "  AAD-SSE2-AllOpt (" << config.numRiskFactors << " RF)...\n";
            XvaResults aadSSE2AllOptResults;
            auto aadSSE2AllOptTiming = runWithTiming(computeForgeAadSSE2, config, swaps, scenarios, eurPillars, today, calendar, dayCounter, aadSSE2AllOptResults);

            // 5. AAD AVX2 variants (if supported)
            XvaResults aadAVX2StabResults, aadAVX2AllOptResults;
            TimingResults aadAVX2StabTiming, aadAVX2AllOptTiming;
            if (avx2Supported) {
                std::cout << "  AAD-AVX2-Stab (" << config.numRiskFactors << " RF)...\n";
                aadAVX2StabTiming = runWithTiming(computeForgeAadAVX2Stability, config, swaps, scenarios, eurPillars, today, calendar, dayCounter, aadAVX2StabResults);

                std::cout << "  AAD-AVX2-AllOpt (" << config.numRiskFactors << " RF)...\n";
                aadAVX2AllOptTiming = runWithTiming(computeForgeAadAVX2, config, swaps, scenarios, eurPillars, today, calendar, dayCounter, aadAVX2AllOptResults);

            }

            // Print results
            printResultsTable(config.name, bumpTiming, bumpResults,
                              forwardSSE2StabTiming, forwardSSE2StabResults,
                              forwardSSE2AllOptTiming, forwardSSE2AllOptResults,
                              avx2Supported ? &forwardAVX2StabTiming : nullptr,
                              avx2Supported ? &forwardAVX2StabResults : nullptr,
                              avx2Supported ? &forwardAVX2AllOptTiming : nullptr,
                              avx2Supported ? &forwardAVX2AllOptResults : nullptr,
                              aadSSE2StabTiming, aadSSE2StabResults,
                              aadSSE2AllOptTiming, aadSSE2AllOptResults,
                              avx2Supported ? &aadAVX2StabTiming : nullptr,
                              avx2Supported ? &aadAVX2StabResults : nullptr,
                              avx2Supported ? &aadAVX2AllOptTiming : nullptr,
                              avx2Supported ? &aadAVX2AllOptResults : nullptr,
                              eurPillars, config);
        }
        ...
        std::cout << "  Comprehensive Forge benchmark completed.\n";
        std::cout << "=============================================================\n";
