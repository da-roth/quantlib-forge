# Local CI Test + QuantLib Tests script (Native Windows)
...
Write-Host "  Local CI Test + QuantLib Tests (Native Windows)" -ForegroundColor Cyan
...
# Build QuantLib with QuantLib-Forge (with ALL tests)
Write-Host "=== Building QuantLib with QuantLib-Forge (with ALL tests) ===" -ForegroundColor Yellow
Push-Location quantlib
...
$forgeTestExe = Get-ChildItem -Recurse -Filter "forge-test-suite.exe" | Select-Object -First 1
if ($forgeTestExe) {
    Write-Host "Running Forge tests from: $($forgeTestExe.FullName)" -ForegroundColor Green
    & $forgeTestExe.FullName --log_level=message
} else {
    Write-Host "Forge test executable not found!" -ForegroundColor Red
    Pop-Location
    exit 1
}
...
Write-Host ""
Write-Host "=============================================================" -ForegroundColor Cyan
Write-Host "  Forge Tests Passed" -ForegroundColor Green
Write-Host "=============================================================" -ForegroundColor Cyan

# Run QuantLib tests
Write-Host ""
Write-Host "=============================================================" -ForegroundColor Cyan
Write-Host "  Running QuantLib Tests (Windows)" -ForegroundColor Cyan
Write-Host "=============================================================" -ForegroundColor Cyan
Write-Host ""

$qlTestExe = Get-ChildItem -Recurse -Filter "quantlib-test-suite.exe" | Select-Object -First 1
if ($qlTestExe) {
    Write-Host "Running QuantLib tests from: $($qlTestExe.FullName)" -ForegroundColor Green
    & $qlTestExe.FullName --log_level=message
} else {
    Write-Host "QuantLib test executable not found!" -ForegroundColor Red
    Pop-Location
    exit 1
}
Pop-Location
